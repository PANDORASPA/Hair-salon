-- PANDORA HEAD SPA starter content.
-- Safe for a fresh project; existing matching rows are updated or left intact where appropriate.
...
INSERT INTO public.settings (key, value) VALUES
  ...
  ('checkout_notice', '完成付款後，套票會加入會員帳戶；如選擇人工付款，需由店員確認後才會發放。'),
  ('fulfillment_note', '套票及預約服務無需配送，到店使用。'),
  ('product_fulfillment_note', '護理產品可到店取貨；如需配送，請先與店員確認。'),
  ...
  ('whatsapp_default_message', '你好，我想查詢 PANDORA HEAD SPA 頭皮護理服務。'),
  ('booking_reminder_note', '請準時到店，如需更改預約請提前聯絡。'),
...
INSERT INTO public.services (name, price, time, category, description, emoji, enabled, sort_order)
SELECT '頭皮水療養護', 880, 90, '高級頭皮護理', '結合頭皮潔淨、保濕與肩頸放鬆，適合想深度休息的客人。', 'SPA', true, 4
WHERE NOT EXISTS (SELECT 1 FROM public.services WHERE name = '頭皮水療養護');

INSERT INTO public.services (name, price, time, category, description, emoji, enabled, sort_order)
SELECT '育髮頭皮管理', 1200, 90, '育髮護理', '針對髮根活力與頭皮狀態，提供定期養護管理方案。', 'GROWTH', true, 5
WHERE NOT EXISTS (SELECT 1 FROM public.services WHERE name = '育髮頭皮管理');
...
INSERT INTO public.products (name, category, description, price, orig, stock, emoji, enabled, sort_order)
SELECT '深層淨化頭皮噴霧', '頭皮護理', '適合外出後或運動後使用，保持頭皮清新。', 220, 280, 18, 'MIST', true, 3
WHERE NOT EXISTS (SELECT 1 FROM public.products WHERE name = '深層淨化頭皮噴霧');
...
SELECT '頭皮護理體驗套票', 1280, 1440, '三次深層頭皮潔淨護理，前台以套票展示；正式扣次以 tickets / user_tickets 為準。', '[]'::jsonb, 'SPA', true
...
SELECT
  ...
  '正式上線前可把這篇示例文章換成真實品牌內容、護理流程與注意事項。',
  true,
  1
...
SELECT
  '預約前需要先註冊會員嗎？',
  '建議先登入會員，因為預約、我的套票和套票扣次都會綁定會員身份。',
  1,
  true
...
SELECT
  '購買套票後可以即時使用嗎？',
  'Stripe 付款成功後會自動發放套票；人工付款需由店員確認後才會加入會員帳戶。',
  2,
  true
...
SELECT
  '使用套票預約後會怎樣扣次？',
  '成功建立預約時會扣 1 次；如取消該預約，系統會回補 1 次並保留紀錄。',
  3,
  true
WHERE NOT EXISTS (SELECT 1 FROM public.faqs WHERE question = '使用套票預約後會怎樣扣次？');

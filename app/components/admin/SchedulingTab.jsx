...
import { addMinutesToTime, parseTimeToMinutes } from '../../../lib/time'
...
const BOARD_SLOT_STEP_MIN = 30
const ACTIVE_BOARD_STATUSES = new Set(['pending', 'confirmed', 'active', 'valid', '有效'])
const CALENDAR_LIMITED_LABEL = '上班 / 規則限制'
const CALENDAR_LIMITED_HINTS = {
  provider_mismatch: '目前所選服務與服務供應者設定未能對上，暫時不會顯示可預約時段。',
  location_required: '此服務需要先確認地點，否則不會建立可預約時段。',
  no_bookable_slots: '有上班，但時段已被休息、休假或封鎖規則扣減。',
  resource_full: '有上班，但資源設備已滿，所以前台會顯示已滿。',
  fully_booked: '有上班，但目前所有可預約時段已被預約。',
}
const cardStyle = {
...
const getBookingDateIso = (booking) => normalizeDateValue(booking?.appointment_date || booking?.date)

const getBookingRange = (booking) => {
  const start = extractTimeText(booking?.start_time || booking?.time)
  if (!start) return null
  const end = extractTimeText(booking?.end_time)
  const durationMin = Number.isFinite(Number(booking?.duration_min)) ? Number(booking.duration_min) : 60
  const startMin = parseTimeToMinutes(start)
  const endText = end || addMinutesToTime(start, durationMin)
  const endMin = parseTimeToMinutes(endText)
  if (startMin == null || endMin == null || endMin <= startMin) return null
  return { start, end: endText, startMin, endMin }
}

const createTimelineSlots = (start, end, stepMin = BOARD_SLOT_STEP_MIN) => {
  const startMin = parseTimeToMinutes(start)
  ...
  for (let current = startMin; current < endMin; current += stepMin) {
    slots.push({
      key: addMinutesToTime('00:00', current),
      startMin: current,
      endMin: Math.min(current + stepMin, endMin),
      label: addMinutesToTime('00:00', current),
    })
  }
  return slots
}
...
const getSelectedDatePlan = ({
  ...
  const overrideHasWorkingWindow = Boolean(override && !override.is_off && override.start_time && override.end_time)
  const overrideInvalid = Boolean(override && !override.is_off && !overrideHasWorkingWindow)
...
export default function SchedulingTab({
  ...
  const boardHours = useMemo(() => parseBusinessHours(settings?.business_hours), [settings?.business_hours])
  const boardTimeline = useMemo(
    () => createTimelineSlots(boardHours.start || DEFAULT_SHIFT_START, boardHours.end || DEFAULT_SHIFT_END),
    [boardHours.end, boardHours.start],
  )
  const timelineStaff = useMemo(
    () => (staff || []).filter((item) => item?.enabled !== false).sort((left, right) => String(left?.name || '').localeCompare(String(right?.name || ''))),
    [staff],
  )
  const selectedDateBookingsByStaff = useMemo(() => {
    const rows = (bookings || [])
      .filter((booking) => getBookingDateIso(booking) === selectedPreviewDate)
      .filter((booking) => ACTIVE_BOARD_STATUSES.has(String(booking?.status || '').toLowerCase()))
      .map((booking) => {
        const staffId = normalizeOptionalNumber(booking?.staff_id)
        const range = getBookingRange(booking)
        if (!staffId || !range) return null
        return {
          id: booking?.id,
          staffId,
          customerName: booking?.customer_name || booking?.name || '顧客',
          serviceName: booking?.service_name || booking?.service || '服務',
    ...
    return rows.reduce((map, booking) => {
      const current = map.get(booking.staffId) || []
      current.push(booking)
      map.set(booking.staffId, current.sort((left, right) => left.startMin - right.startMin))
      return map
    }, new Map())
  }, [bookings, selectedPreviewDate])
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      if (resolvedStaffId !== previousStaffId) {
        ...
        setBreakRows((current) => remapRows(current))
        setTimeOffRows((current) => remapRows(current))
        setBlockedRows((current) => remapRows(current))
      }
  ...
  if (!staff.length) {
    return (
      <div style={{ display: 'grid', gap: '18px' }}>
        <SectionHeader eyebrow="服務供應者" title="排班設定" description="先建立服務供應者，之後即可設定每週上班時間、日期覆蓋、固定休息、休假與封鎖時段。" />
        <EmptyState
          title="尚未建立服務供應者"
      ...
      <SectionHeader
        ...
        actions={
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            <button type="button" className="btn btn-secondary btn-interactive" onClick={onAddStaff} disabled={saving}>新增服務供應者</button>
            <button type="button" className="btn btn-interactive" onClick={saveAll} disabled={saving || !selectedStaff}>{saving ? '儲存中...' : '儲存目前服務供應者'}</button>
          </div>
        }
      />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : '280px minmax(0, 1fr)', gap: '18px', alignItems: 'start' }}>
        <div className="admin-card" style={{ padding: '16px', border: '1px solid #E5E7EB', display: 'grid', gap: '10px' }}>
          {staff.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => setSelectedStaffId(item.id)} className="btn-interactive" style={{ ...cardStyle, padding: '14px', border: selected ? '1px solid rgba(166, 139, 106, 0.45)' : '1px solid #E5E7EB', background: selected ? 'rgba(166, 139, 106, 0.08)' : '#fff', textAlign: 'left' }}>
          ...
          <div style={{ display: 'grid', gap: '18px' }}>
            <div className="admin-card" style={{ ...cardStyle, display: 'grid', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
                <label style={{ display: 'grid', gap: '8px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>姓名</span><input value={selectedStaff.name || ''} onChange={(event) => onUpdateField(selectedStaff.id, 'name', event.target.value)} style={fieldStyle} /></label>
                <label style={{ display: 'grid', gap: '8px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>角色</span><input value={selectedStaff.role || ''} onChange={(event) => onUpdateField(selectedStaff.id, 'role', event.target.value)} style={fieldStyle} /></label>
                <label style={{ display: 'grid', gap: '8px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>電話</span><input value={selectedStaff.phone || ''} onChange={(event) => onUpdateField(selectedStaff.id, 'phone', event.target.value)} style={fieldStyle} /></label>
                <label style={{ display: 'grid', gap: '8px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>狀態</span><button type="button" className="btn btn-secondary btn-interactive" onClick={() => onUpdateField(selectedStaff.id, 'enabled', selectedStaff.enabled === false)}>{selectedStaff.enabled === false ? '目前停用，按此啟用' : '目前啟用，按此停用'}</button></label>
              </div>
              <label style={{ display: 'grid', gap: '8px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>簡介</span><textarea value={selectedStaff.bio || ''} onChange={(event) => onUpdateField(selectedStaff.id, 'bio', event.target.value)} style={{ ...fieldStyle, minHeight: '90px', resize: 'vertical' }} /></label>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {services.map((service) => {
                  const checked = (selectedStaff.services || []).map(String).includes(String(service.id))
                  return <label key={service.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '10px 12px', borderRadius: '999px', border: '1px solid #E5E7EB', background: checked ? 'rgba(166, 139, 106, 0.12)' : '#fff', cursor: 'pointer' }}><input type="checkbox" checked={checked} onChange={() => onToggleService(selectedStaff.id, service.id)} /><span style={{ fontSize: '13px', fontWeight: 700 }}>{service.name}</span></label>
                })}
              </div>
              <button type="button" className="btn btn-danger btn-interactive" onClick={() => onDeleteStaff(selectedStaff.id)} disabled={saving}>刪除這位服務供應者</button>
            </div>

            <div className="admin-card" style={{ ...cardStyle, display: 'grid', gap: '16px' }}>
              <div><div style={{ fontSize: '12px', color: '#A68B6A', fontWeight: 800, letterSpacing: '0.08em' }}>每週時間表</div><div style={{ marginTop: '4px', fontSize: '20px', fontWeight: 900 }}>設定上班時間與下班時間</div><div style={{ marginTop: '6px', color: '#6B7280', fontSize: '13px', lineHeight: 1.6 }}>固定休息、休假與封鎖時段會再扣減可預約時間。</div></div>
              <div style={{ display: 'grid', gridTemplateColumns: compact ? '1fr' : 'repeat(7, minmax(160px, 1fr))', gap: '12px' }}>
                {WEEK_DAYS.map((day) => {
                  ...
                  return (
                    <div key={day.key} style={{ ...cardStyle, padding: '14px', display: 'grid', gap: '12px', background: isOff ? '#F8FAFC' : '#fff' }}>
                      <div style={{ fontSize: '18px', fontWeight: 900 }}>{day.label}</div>
                      <div style={{ display: 'inline-flex', gap: '6px', flexWrap: 'wrap' }}><button type="button" onClick={() => onSetDailyMode(selectedStaff.id, day.key, 'work')} style={dayModeButtonStyle(!isOff)}>上班</button><button type="button" onClick={() => onSetDailyMode(selectedStaff.id, day.key, 'rest')} style={dayModeButtonStyle(isOff)}>休息</button></div>
                      <label style={{ display: 'grid', gap: '6px' }}><span style={{ fontSize: '12px', fontWeight: 700 }}>上班時間</span><input type="time" value={formatTime(schedule.start)} onChange={(event) => onUpdateSchedule(selectedStaff.id, day.key, 'start', event.target.value)} style={fieldStyle} disabled={isOff} /></label>
                      <label style={{ display: 'grid', gap: '6px' }}><span style={{ fontSize: '12px', fontWeight: 700 }}>下班時間</span><input type="time" value={formatTime(schedule.end)} onChange={(event) => onUpdateSchedule(selectedStaff.id, day.key, 'end', event.target.value)} style={fieldStyle} disabled={isOff} /></label>
                      <Pill tone={isOff ? 'muted' : hasWorkingHours ? 'success' : 'warning'}>{isOff ? '休息' : hasWorkingHours ? '上班' : '未設定'}</Pill>
                    </div>
                  )
                })}
              </div>
            ...
            <RowsEditor title="日期覆蓋" description="用來設定特定日期上班、下班或整日休息。" emptyDescription="當某一天需要特別調整上班或休息時，先新增一筆日期覆蓋。" rows={shiftRows} onAdd={() => setShiftRows((current) => [...current, { id: nextTempId(), staff_id: selectedStaff.id, date: todayISO(), start_time: DEFAULT_SHIFT_START, end_time: DEFAULT_SHIFT_END, is_off: false }])} onRemove={(id) => removeRow(setShiftRows, setShiftDeletedIds, id)} setRows={setShiftRows} compact={compact} renderRow={(row, update) => <><input type="date" value={row.date || ''} onChange={(event) => update({ date: event.target.value })} style={smallFieldStyle} /><input type="time" value={row.start_time || ''} onChange={(event) => update({ start_time: event.target.value })} style={smallFieldStyle} disabled={row.is_off} /><input type="time" value={row.end_time || ''} onChange={(event) => update({ end_time: event.target.value })} style={smallFieldStyle} disabled={row.is_off} /><button type="button" onClick={() => update({ is_off: !row.is_off, start_time: row.is_off ? DEFAULT_SHIFT_START : '', end_time: row.is_off ? DEFAULT_SHIFT_END : '' })} style={dayModeButtonStyle(Boolean(row.is_off))}>{row.is_off ? '休息' : '上班'}</button></>} />
            <RowsEditor title="固定休息時段" description="每週固定會扣減的休息時段。" emptyDescription="例如：午飯時間、茶點時間或固定小休。" rows={breakRows} onAdd={() => setBreakRows((current) => [...current, { id: nextTempId(), staff_id: selectedStaff.id, day_of_week: '1', start_time: '13:00', end_time: '14:00', label: '', enabled: true }])} onRemove={(id) => removeRow(setBreakRows, setBreakDeletedIds, id)} setRows={setBreakRows} compact={compact} renderRow={(row, update) => <><select value={row.day_of_week ?? '1'} onChange={(event) => update({ day_of_week: event.target.value })} style={smallFieldStyle}>{WEEK_DAYS.map((day) => <option key={day.key} value={day.key}>{day.label}</option>)}</select><input type="time" value={row.start_time || ''} onChange={(event) => update({ start_time: event.target.value })} style={smallFieldStyle} /><input type="time" value={row.end_time || ''} onChange={(event) => update({ end_time: event.target.value })} style={smallFieldStyle} /><input value={row.label || ''} onChange={(event) => update({ label: event.target.value })} style={smallFieldStyle} placeholder="說明" /></>} />
            <RowsEditor title="休假時段" description="用來設定特定日期的全日或部分休假。" emptyDescription="當某一天需要放假，或只封鎖某一段時間，可在這裡處理。" rows={timeOffRows} onAdd={() => setTimeOffRows((current) => [...current, { id: nextTempId(), staff_id: selectedStaff.id, date: todayISO(), start_time: DEFAULT_SHIFT_START, end_time: DEFAULT_SHIFT_END, reason: '', is_all_day: false }])} onRemove={(id) => removeRow(setTimeOffRows, setTimeOffDeletedIds, id)} setRows={setTimeOffRows} compact={compact} renderRow={(row, update) => <><input type="date" value={row.date || ''} onChange={(event) => update({ date: event.target.value })} style={smallFieldStyle} /><input type="time" value={row.start_time || ''} onChange={(event) => update({ start_time: event.target.value })} style={smallFieldStyle} disabled={row.is_all_day} /><input type="time" value={row.end_time || ''} onChange={(event) => update({ end_time: event.target.value })} style={smallFieldStyle} disabled={row.is_all_day} /><label style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#6B7280' }}><input type="checkbox" checked={Boolean(row.is_all_day)} onChange={(event) => update(event.target.checked ? { is_all_day: true, start_time: '00:00', end_time: '23:59' } : { is_all_day: false, start_time: DEFAULT_SHIFT_START, end_time: DEFAULT_SHIFT_END })} />全天</label></>} />
            <RowsEditor title="封鎖時段" description="用來封鎖特定日期和時段，避免前台出現可預約狀態。" emptyDescription="例如會議、內部工作、設備保養等不能預約的時間。" rows={blockedRows} onAdd={() => setBlockedRows((current) => [...current, { id: nextTempId(), staff_id: selectedStaff.id, date: todayISO(), start_time: '11:00', end_time: '12:00', reason: '', source: 'manual' }])} onRemove={(id) => removeRow(setBlockedRows, setBlockedDeletedIds, id)} setRows={setBlockedRows} compact={compact} renderRow={(row, update) => <><input type="date" value={row.date || ''} onChange={(event) => update({ date: event.target.value })} style={smallFieldStyle} /><input type="time" value={row.start_time || ''} onChange={(event) => update({ start_time: event.target.value })} style={smallFieldStyle} /><input type="time" value={row.end_time || ''} onChange={(event) => update({ end_time: event.target.value })} style={smallFieldStyle} /><input value={row.reason || ''} onChange={(event) => update({ reason: event.target.value })} style={smallFieldStyle} placeholder="原因" /></>} />

            <div className="admin-card" style={{ ...cardStyle, display: 'grid', gap: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div><div style={{ fontSize: '12px', color: '#A68B6A', fontWeight: 800, letterSpacing: '0.08em' }}>前台可預約對照</div><div style={{ marginTop: '4px', fontSize: '20px', fontWeight: 900 }}>月曆預覽</div><div style={{ marginTop: '6px', color: '#6B7280', fontSize: '13px', lineHeight: 1.6 }}>與前台共用同一份月曆摘要，直接對照可預約、已滿與休息。</div></div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}><button type="button" className="btn btn-small btn-interactive" onClick={() => setPreviewMonth((current) => addMonths(current, -1))} disabled={previewLoading}>上月</button><button type="button" className="btn btn-small btn-interactive" onClick={() => setPreviewMonth((current) => addMonths(current, 1))} disabled={previewLoading}>下月</button></div>
              </div>
              <label style={{ display: 'grid', gap: '8px', maxWidth: '360px' }}><span style={{ fontSize: '13px', fontWeight: 800 }}>對照服務</span><select value={previewServiceId} onChange={(event) => setPreviewServiceId(event.target.value)} style={fieldStyle}>{services.length ? services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>) : <option value="">未指定</option>}</select></label>
              ...
              <div className="hide-scrollbar" style={{ overflowX: 'auto' }}>
                <div style={{ minWidth: compact ? '700px' : 'auto', display: 'grid', gap: '10px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: '8px' }}>{['一', '二', '三', '四', '五', '六', '日'].map((weekday) => <div key={weekday} style={{ textAlign: 'center', fontSize: '12px', color: '#6B7280', fontWeight: 800 }}>星期{weekday}</div>)}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: '10px' }}>
                    {previewGrid.map(({ dateISO, inMonth, dayLabel }) => { const entry = previewMap.get(dateISO); const status = entry?.status || 'off'; const previewCopy = getPreviewStatusCopy(entry); const countLabel = status === 'available' ? `${Number(entry?.availableCount || 0)} 個可約時段` : status === 'full' && Number(entry?.slotCount || 0) > 0 ? `${Number(entry?.slotCount || 0)} 個工作時段` : ''; const isSelected = selectedPreviewDate === dateISO; return <button key={dateISO} type="button" onClick={() => { if (!inMonth) return; setSelectedPreviewDate(dateISO) }} style={{ ...cardStyle, minHeight: '92px', padding: '10px', opacity: inMonth ? 1 : 0.45, color: status === 'off' ? '#9CA3AF' : '#111827', background: isSelected ? '#FFF8EE' : status === 'off' ? '#F8FAFC' : '#fff', borderColor: isSelected ? '#A68B6A' : status === 'full' ? 'rgba(166, 139, 106, 0.45)' : '#E5E7EB', textAlign: 'left', cursor: inMonth ? 'pointer' : 'default' }}><div style={{ fontSize: '15px', fontWeight: 900 }}>{dayLabel}</div>{inMonth ? <div style={{ marginTop: '8px', display: 'grid', gap: '6px' }}><Pill tone={previewCopy.tone}>{previewCopy.label}</Pill>{countLabel ? <div style={{ fontSize: '12px', fontWeight: 800, color: status === 'off' ? '#9CA3AF' : '#374151' }}>{countLabel}</div> : null}<div style={{ fontSize: '12px', fontWeight: 700, color: status === 'off' ? '#9CA3AF' : '#6B7280', lineHeight: 1.4 }}>{previewCopy.hint}</div></div> : null}</button> })}
                  </div>
                </div>
              </div>
            </div>
